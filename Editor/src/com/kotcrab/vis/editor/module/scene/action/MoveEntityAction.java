...
public class MoveEntityAction implements UndoableAction {
	...
	private void reload () {
		if (entity.getEntity().isActive() == false) {
			EntityProxyCache proxyCache = entity.getEntity().getWorld().getManager(EntityProxyCache.class);
			entity = proxyCache.get(entity.getEntity().getWorld().getEntity(entity.getEntity().getId()));
		}
	}
