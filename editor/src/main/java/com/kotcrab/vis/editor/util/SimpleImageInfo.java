...
public class SimpleImageInfo {
	...
	public SimpleImageInfo (File file) throws IOException, EditorException {
		try (InputStream is = new FileInputStream(file)) {
			processStream(is);
		}
	}

	public SimpleImageInfo (InputStream is) throws IOException, EditorException {
		processStream(is);
	}

	public SimpleImageInfo (byte[] bytes) throws IOException, EditorException {
		try (InputStream is = new ByteArrayInputStream(bytes)) {
			processStream(is);
		}
	}

	private void processStream (InputStream is) throws IOException, EditorException {
		int c1 = is.read();
		int c2 = is.read();
		int c3 = is.read();
		...
		if (mimeType == null) {
			throw new EditorException("Unsupported image type");
		}
	}
